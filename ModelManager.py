...
class ModelManager():
    ...
    def predict_all(self, image_dir):
        ...
        Params:
            image_dir (list): list of paths to the images
        """

        images = os.listdir(image_dir)

        # define paths for correct and incorrect directories
        correct_dir_path = os.path.join(self.video_manager.processed_path, self.correct_dir)
        incorrect_dir_path = os.path.join(self.video_manager.processed_path, self.incorrect_dir)
        ...
        for image in images:
            ...
            extracted_number = image.split('_')[-1].split('.')[0]

            # process the image if the extracted number is a multiple of frame_skip
            if int(extracted_number) % self.frame_skip == 0:
                
                # use model on the image and get bounding box values
                bbox_values = self.model(os.path.join(image_dir, image), conf=self.conf_threshold)[0].boxes
                ...
                if num_of_objects < 1:
                    ...
                    shutil.copy(os.path.join(image_dir, image), incorrect_dir_path)
                    ...
                    shutil.copy(os.path.join(image_dir, image), correct_dir_path)
    ...
    def retrain(self):
        ...
        video_paths = []
        print("""\nSelect video files you want to use to train, you can select more than one. These video files must have been annotated previously and have annotations saved in their directory.\n
    Press Cancel or exit out of the file explorer when finished choosing videos.
        ...
        # add a way to change params depending on how the input size, or to user request
    ...
    def predicting(self):
        ...
        # get prediction
        bbox_values = self.model.track(self.img_path, conf=self.conf_threshold)[0].boxes
