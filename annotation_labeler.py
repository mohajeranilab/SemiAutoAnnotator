...
def clustering_data(image_dir, model_dir):
   initialize_clustering(Path(image_dir), model_dir)
   pass
...
def show_image(): 
    ...
    ***Moving to a specific place solved my issue of the cv2 window opening in random parts of the screen 
    """
    global window_info
    if not any(value is None for value in window_info.values()):
            ...
            if gw.getWindowsWithTitle(img_name):
                ...
                window_info = {}
                window_info["img_name"] = img_name
                window_info["coordinates"] = (x, y)
                window_info["dimensions"] = (window_width, window_height)

            else:
                
                x, y = window_info['coordinates']
    ...
    if window_info["img_name"] != img_name:
    ...
    if flag:
        cv2.resizeWindow(img_name, (1, 1))
        cv2.moveWindow(img_name, -5000, -5000)
...
def annotating(img_path, img_name, video_extraction_dir):
    ...
    while True:
        ...
        if key == 27 or cv2.getWindowProperty(img_name, cv2.WND_PROP_VISIBLE) < 1: # "Escape": Exits the program 
            ...
            object_id -= 1 if object_id > 1 else 0 
            ...
            cv2.destroyWindow(window_info['img_name'])
...
if __name__ == "__main__":
    ...
    window_info = {"img_name": None, "coordinates": None, "dimensions": None}
